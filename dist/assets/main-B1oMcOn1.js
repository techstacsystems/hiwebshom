import{i as v,u as h,e as d,f as y,h as _,s as w}from"./utils-Dixr07uC.js";import{d as g}from"./firebase-config-B8-7fj8g.js";import{query as k,collection as x,orderBy as E,getDocs as j}from"https://www.gstatic.com/firebasejs/10.12.0/firebase-firestore.js";import"https://www.gstatic.com/firebasejs/10.12.0/firebase-app.js";import"https://www.gstatic.com/firebasejs/10.12.0/firebase-auth.js";import"https://www.gstatic.com/firebasejs/10.12.0/firebase-storage.js";v();h();const n=document.getElementById("products-grid"),p=document.getElementById("no-products"),c=document.getElementById("filters");let i=[],l="all";async function $(){if(P()){n.innerHTML="",f();return}const e=new Promise((t,o)=>setTimeout(()=>o(new Error("timeout")),8e3));try{const t=k(x(g,"products"),E("createdAt","desc"));i=(await Promise.race([j(t),e])).docs.map(r=>({id:r.id,...r.data()})),z(),b(i)}catch(t){console.error("Greška:",t),n.innerHTML="",t.message==="timeout"||t.code==="unavailable"||t.code==="permission-denied"?f():p.style.display="block"}}function P(){try{const e=g.app;return e.options.apiKey==="YOUR_API_KEY"||!e.options.projectId||e.options.projectId==="YOUR_PROJECT_ID"}catch{return!1}}function f(){n.innerHTML=`
  ...
  `,s.addEventListener("click",u=>{if(u.target.closest(".product-card__add")){u.stopPropagation(),t||(_(e,1),w(`${e.name} dodano u korpu`));return}window.location.href=`.//hiwebshom/product.html?id=${e.id}`}),s}$();
