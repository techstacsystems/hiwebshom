import{i as v,u as h,e as d,f as y,h as _,s as w}from"./utils-Dixr07uC.js";import{d as g}from"./firebase-config-B8-7fj8g.js";import{query as k,collection as x,orderBy as E,getDocs as j}from"https://www.gstatic.com/firebasejs/10.12.0/firebase-firestore.js";import"https://www.gstatic.com/firebasejs/10.12.0/firebase-app.js";import"https://www.gstatic.com/firebasejs/10.12.0/firebase-auth.js";import"https://www.gstatic.com/firebasejs/10.12.0/firebase-storage.js";v();h();const s=document.getElementById("products-grid"),p=document.getElementById("no-products"),c=document.getElementById("filters");let i=[],l="all";async function $(){if(P()){s.innerHTML="",f();return}const e=new Promise((t,o)=>setTimeout(()=>o(new Error("timeout")),8e3));try{const t=k(x(g,"products"),E("createdAt","desc"));i=(await Promise.race([j(t),e])).docs.map(r=>({id:r.id,...r.data()})),z(),b(i)}catch(t){console.error("Greška:",t),s.innerHTML="",t.message==="timeout"||t.code==="unavailable"||t.code==="permission-denied"?f():p.style.display="block"}}function P(){try{const e=g.app;return e.options.apiKey==="YOUR_API_KEY"||!e.options.projectId||e.options.projectId==="YOUR_PROJECT_ID"}catch{return!1}}function f(){s.innerHTML=`
  ...
  `}function z(){[...new Set(i.map(t=>t.category).filter(Boolean))].forEach(t=>{const o=document.createElement("button");o.className="filter-btn",o.dataset.filter=t,o.textContent=t,c.appendChild(o)}),c.addEventListener("click",t=>{const o=t.target.closest(".filter-btn");if(!o)return;c.querySelectorAll(".filter-btn").forEach(a=>a.classList.remove("active")),o.classList.add("active"),l=o.dataset.filter;const r=l==="all"?i:i.filter(a=>a.category===l);b(r)})}function b(e){if(s.innerHTML="",!e.length){p.style.display="block";return}p.style.display="none",e.forEach(t=>s.appendChild(C(t)))}function C(e){var m;const t=e.stock<=0,o=e.stock>0&&e.stock<=5,r=e.createdAt&&Date.now()-e.createdAt.toMillis()<7*24*60*60*1e3,a=e.ml?`<span style="font-size:0.72rem;color:var(--color-text-light);font-weight:400;margin-left:4px;">(${e.ml} ml)</span>`:"",n=document.createElement("div");return n.className="product-card",n.innerHTML=`
    ...
    <div class="product-card__body">
      ...
      <div class="product-card__footer">
        <span class="product-card__price">${y(e.price)}</span>
        <button class="product-card__add" data-id="${e.id}" aria-label="Dodaj u korpu" ${t?"disabled":""}>
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15" /></svg>
        </button>
      </div>
    </div>
  `,n.addEventListener("click",u=>{if(u.target.closest(".product-card__add")){u.stopPropagation(),t||(_(e,1),w(`${e.name} dodano u korpu`));return}window.location.href=`.//hiwebshom//hiwebshom/product.html?id=${e.id}`}),n}$();
